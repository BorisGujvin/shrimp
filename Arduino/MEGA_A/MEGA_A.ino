      ...
      case 112://-----'p'------:
        switch (N1){
          ...
          case 2:
            T2_ = d.toFloat();
            EEPROM.put(4,T2_);
            Serial.println("p2 set");
            break;
          case 3:
            T3_ = d.toFloat();
            EEPROM.put(8,T3_);
            Serial.println("p3 set");
            break;
         };
        break;
      case 107://------'k'-------:
...
float getTemp2(void) {
  byte i;byte present = 0;byte type_s; byte data[12];float celsius;
  ds2.reset();ds2.select(addr2);ds2.write(0x44, 1);delay(1000);present = ds2.reset();ds2.select(addr2);ds2.write(0xBE);for ( i = 0; i < 9; i++) {data[i] = ds2.read();}int16_t raw = (data[1] << 8) | data[0];
  if (type_s) {raw = raw << 3;  if (data[7] == 0x10) {raw = (raw & 0xFFF0) + 12 - data[6];}} else {byte cfg = (data[4] & 0x60);if (cfg == 0x00) raw = raw & ~7; else if (cfg == 0x20) raw = raw & ~3;
  else if (cfg == 0x40) raw = raw & ~1; } celsius = (float)raw / 16.0;return(celsius);
   }
float getTemp3(void) {
  byte i;byte present = 0;byte type_s; byte data[12];float celsius;
  ds3.reset();ds3.select(addr3);ds3.write(0x44, 1);delay(1000);present = ds3.reset();ds3.select(addr3);ds3.write(0xBE);for ( i = 0; i < 9; i++) {data[i] = ds3.read();}int16_t raw = (data[1] << 8) | data[0];
  if (type_s) {raw = raw << 3;  if (data[7] == 0x10) {raw = (raw & 0xFFF0) + 12 - data[6];}} else {byte cfg = (data[4] & 0x60);if (cfg == 0x00) raw = raw & ~7; else if (cfg == 0x20) raw = raw & ~3;
  else if (cfg == 0x40) raw = raw & ~1; } celsius = (float)raw / 16.0;return(celsius);
   }
...
void SendData(){
 ...
 do {
    ...
    sprintf(mbuffer, "GET /datatrans.php?pass=ardu&retry=%d&p1=%s&p2=%s&p3=%s HTTP/1.1\r\nHost: shrimp.pp.ua\r\n\r\n",retr,tmp,tmp1,tmp2);//GET /datatrans.php?pass=ardu&retry=%d&p1=%s&p2=%s&p3=%s HTTP/1.1\r\nHost: shrimp.pp.ua\r\n\r\n
    int j=1;
...
void seekTerm(){
    while ( !ds.search(addr)){  MyLCD("No term1((","");delay(500);};
    while ( !ds2.search(addr2)){  MyLCD("No term2((","");delay(500);};
    while ( !ds3.search(addr3)){  MyLCD("No term3((","");delay(500);};
  MyLCD("Terms","found");
} 
void actualize(){
  ...
  //  get_data();
    T1_cur = getTemp();
    Serial.print("T1=");Serial.println(T1_cur);
    if (T1_cur<T1_)digitalWrite (K1, LOW);
    if (T1_cur>T1_+D1_)digitalWrite (K1, HIGH);
    T2_cur = getTemp2();
    Serial.print("T2=");Serial.println(T2_cur);

    if (T2_cur<T2_)digitalWrite (K3, LOW);
    if (T2_cur>T2_+D1_)digitalWrite (K3, HIGH);
    T3_cur = getTemp3();
    Serial.print("T3=");Serial.println(T3_cur);

    if (T3_cur<T3_)digitalWrite (K4, LOW);
    if (T3_cur>T3_+D1_)digitalWrite (K4, HIGH);
    Serial.println(rtc.getTimeStr());
    String T1S=String(T1_cur);
    String T2S=String(T2_cur);
    String T3S=String(T3_cur);
    String F1=" ";
    String F2=" ";
    String F3=" ";
    if(!digitalRead(K1))F1="^";
    if(!digitalRead(K3))F2="^";
    if(!digitalRead(K4))F3="^";
    T1S=T1S.substring(0,4);
    T2S=T2S.substring(0,4);
    T3S=T3S.substring(0,4);
     String i1 = T2S+char(223)+F2+"    "+T3S+char(223)+F3;
     String i2 = rtc.getTimeStr();
     i2=i2.substring(0,5);
     i2=T1S+char(223)+F1+"     "+i2;
     MyLCD(i1,i2);
}
void mDelay(int w){
 do{
 ...
 if (x < 60) {
     ...
     Serial.print("Select ");Serial.print(digitalRead(K2));
