...
void setup(void){
  ...
  printf_begin();
  seekTerm();
  printf("\n\rпрограмма для аквариума на первом этаже \n\r");
  radio.begin();
...
void loop(void)
{
    if ( radio.available() )
    {
      ...
      while (!done)
      {
       ...
       if(got_time==1){digitalWrite(K2, LOW);};
       if(got_time==0){digitalWrite(K2, HIGH);};
       byte op = got_time;
       if((op & B10000000)!=0){
         op = op & B01111111;
         T1=float(op);
   ...
   /* if(Tmp>0)*/ T1_cur  = Tmp;
    ...
    if (T1_cur>(T1+D)) {digitalWrite(K1, HIGH);Serial.println("T Off");};
    if (T1_cur<T1)  { digitalWrite(K1,LOW);Serial.println("T On");};
}
